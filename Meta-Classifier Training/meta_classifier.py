...
def plot_graph(accuracies):
    user_data_volume = [i * 20 for i in range(1, 11)]

    plt.plot(user_data_volume, accuracies)
    plt.xlabel('User Data Volume')
    plt.ylabel('Accuracy')
    plt.title('User Data Volume vs Accuracy')
    plt.grid()
    plt.show()
...
def load_data(data_path):
    ...
    for i in range(10):
        for j in range(100):
            read_list_path = data_path + f"/train_data/num/{i}/0/{j}.jpg"
            x_list_temp = img.imread(read_list_path)
            x_train_attack.append(x_list_temp)
            y_train_attack.append(i)
    for i in range(10):
        for j in range(20):
            read_list_path = data_path + f"/test_data/num/{i}/0/{j}.jpg"
            x_list_temp = img.imread(read_list_path)
